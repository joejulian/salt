...
def create_ca(
    ...
    already exists unless you set force to true.
    ...
    '''
    if _ca_exists(ca_name) and not force:
...
def create_csr(
    ...
    if not _ca_exists(ca_name) and not force:
    ...
    if not force and os.path.exists('{0}/{1}/certs/{2}.csr'.format(
            _cert_base_path(),
            ca_name,
            CN)
            ):
...
def create_self_signed_cert(
    ...
    exists, the function just returns unless force is set.
...
def create_ca_signed_cert(ca_name, CN, days=365, force=False):
    '''
...
def create_pkcs12(ca_name, CN, passphrase='', force=False):
    '''
